...
int main() {
  ...
  printf("solving 4x4 system...\n");
  LUdecomp *LU3 = LUdecompose(2, A2);
  double x2[2];
  LUsolve(LU3, b2, x2);
  double x2_soln[2] = {
    -1000000.0/999999, 999998.0/999999
  };

  for (int i = 0; i < 2; i++) {
    const double err = x2[i] - x2_soln[i];
  ...
  //
  printf("solving 4x4 system...\n");
  LUdecomp *LU2 = LUdecompose(4, A4);
  double x4[4];
  LUsolve(LU2, b4, x4);
  double x4_soln[4] = {
    -97.0/63, 131.0/63, 19.0/9, -61.0/21
  };

  for (int i = 0; i < 4; i++) {
    const double err = x4[i] - x4_soln[i];
    printf("x[i] = %11.7f (%11.7f, error=%0.7e)\n", 
     x4[i], x4_soln[i], err);
  }
